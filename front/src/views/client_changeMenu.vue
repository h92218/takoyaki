<template>
    ...
    <input type="submit" @click="submit" value="주문변경 완료">
    </div>
...
export default {
  ...
  mounted(){
    ...
    let menus = this.$route.query.menu;
    let flavors = this.$route.query.flavor;
    let menu = menus.split(' ');
    console.log(menu);
    let flavor = flavors.split(' ');
    for(let i=1; i<menu.length;i++){
        this.selectedMenu.push(menu[i]);
    }
    for(let i=1; i<flavor.length;i++){
        this.selectedFlavor.push(flavor[i]);
    }
  ...
  methods:{
    submit(){
            ...
            console.log(obj);
             fetch("http://localhost:8081/changeMenu",{
                                        method : 'POST',
                                        mode : 'cors',
                                        cache : 'no-cache',
